...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		obtenerVecinos(tablero, vecinos, pos);
        ...
        if (ceroEsVecino) {
            int posCero = posicion(tablero, 0);
            
            int temp = tablero[pos];
            tablero[pos] = tablero[posCero];
            tablero[posCero] = temp;
        } else {
            // TODO: Y si no, decir que ño
            printf("\nEsa pieza no se pude mover\n\n");
            getch();  
        }
